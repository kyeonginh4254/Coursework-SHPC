...
/* Conv1D CUDA Kernel */
__global__ void Conv1d_Kernel(float *global_in, float *global_w,
                              float *global_out, float *global_b,
                              size_t s, size_t C, size_t OC, size_t K) {
  size_t i = blockIdx.x;    // output channel index (0 ~ OC-1)
  size_t j = threadIdx.x;   // output sequence index (0 ~ s-1)
  ...
  if (j < os) {
    ...
    for (size_t c = 0; c < C; c++) {
      for (size_t l = 0; l < K; l++) {
        val += global_in[c * s + (j + l)] * global_w[i * C * K + c * K + l];
      }
    }
    global_out[i * os + j] = val + shared_b;
  }
}
...
void Conv1D_CUDA(Tensor *in, Tensor *w, Tensor *b, Tensor *out) {
  ...
  Conv1d_Kernel<<<OC, s>>>(d_in, d_w, d_out, d_b, s, C, OC, K);
