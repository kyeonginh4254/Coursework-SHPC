...
void Linear_ReLU_CUDA(Tensor *in, Tensor *w, Tensor *b, Tensor *out) {
  ...
  CHECK_CUDA(cudaMalloc(&d_wt, M * N * sizeof(float)));
  ...
  CHECK_CUDA(cudaMemcpy(d_wt, w->buf, M * N * sizeof(float), cudaMemcpyHostToDevice));
  ...
  Linear_ReLU_CUDA_Kernel<<<grid, block>>>(d_in, d_wt, d_out, d_b, B, M, N);
...
void Linear_CUDA(Tensor *in, Tensor *w, Tensor *b, Tensor *out) {
  ...
  Linear_CUDA_Kernel<<<grid, block>>>(d_in, d_wt, d_out, d_b, B, M, N);
