...
void predict_sentiment(int *inputs, float *outputs, size_t n_samples) {
    ...
    if (mpi_rank == 0) {
        ...
        for (size_t n = 0; n < n_samples; n += BATCH_SIZE) {
            ...
            Linear_ReLU_CUDA(concat_a, linear0_w, linear0_b, linear0_a);
            STOP_TIMER(start, stop, elapsed);
            ...
            Linear_ReLU_CUDA(linear1_a, linear2_w, linear2_b, linear2_a);
            STOP_TIMER(start, stop, elapsed);
            ...
            Linear_CUDA(linear2_a, linear3_w, linear3_b, linear3_a);
            STOP_TIMER(start, stop, elapsed);
