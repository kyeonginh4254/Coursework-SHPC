...
static cudaStream_t streams_knl[NUM_DEVICE];
static cudaStream_t streams_mem[NUM_DEVICE];
cudaEvent_t events_htod[NUM_DEVICE], events_dtoh[NUM_DEVICE];
...
__global__ void matmul_kernel(const float *A, const float *B, float *C, int M, int N, int K) {
  ...
  const int row = threadIdx.x; // Local row index within the block
  const int col = threadIdx.y; // Local column index within the block
...
void matmul(const float *A, const float *B, float *C, int M, int N, int K) {
  ...
  for (int i = 0; i < num_devices; i++) {
    ...
    dim3 gridDim(CEIL_DIV(M_i_size, TS), CEIL_DIV(N, TS));
    dim3 blockDim(RTS, RTS);

    matmul_kernel<<<gridDim, blockDim, 0, streams_knl[i]>>>(a_d[i], b_d[i], c_d[i], M_i_size, N, K);
  ...
  for (int i = 0; i < num_devices; i++) {
    CUDA_CALL(cudaSetDevice(i));
    CUDA_CALL(cudaStreamSynchronize(streams_knl[i]));
  }
